"""
Eurostat Wages & Labor Ingestor

Sources:
  une_rt_m   Monthly unemployment rate (LFS, ages 15-74, both sexes)
  lc_lci_r2  Labour Cost Index (total economy, NSA, quarterly)

Output: lekwankwa-historical-vault/product=wages_and_employment/
            country={ISO3}/source=eurostat_sdmx/year=YYYY/month=MM/
            wages_data.parquet

Schema matches SCHEMA_GOLD_STANDARD wages.json field names.
"""
...
from scrapers.eurostat.series_map import WAGES_CONFIGS
...
def _build_wages_rows(df_raw: pd.DataFrame, cfg: dict) -> pd.DataFrame:
    """Map raw API observations to wages_and_employment schema."""
    if df_raw.empty:
        return pd.DataFrame()
    ...
    # Determine geo column name (Eurostat may use "geo" or "geo\TIME_PERIOD")
    ...
    for _, r in df_raw.iterrows():
        ...
        rows.append({
            # Gold-standard wages.json fields
            "data_vintage_id":       vid,
...
def _ingest_one(cfg: dict) -> int:
    """Fetch one wages/labor series and write to vault."""
    dataflow    = cfg["dataflow"]
    metric_code = cfg["metric_code"]
    start       = cfg.get("start_period", "2000-01")
    ...
    df_vault = _build_wages_rows(df_raw, cfg)
...
def run() -> int:
    log.info("=" * 70)
    log.info("EUROSTAT — Wages & Labor")
    log.info(f"Series: {len(WAGES_CONFIGS)} | Countries: {len(ALL_GEO2)}")
    log.info("=" * 70)

    total = 0
    for cfg in WAGES_CONFIGS:
        total += _ingest_one(cfg)

    log.info(f"\nWages & labor ingestion complete: {total:,} rows")
    return total
