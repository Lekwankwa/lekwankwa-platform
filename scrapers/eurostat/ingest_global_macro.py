"""
Eurostat Global Macro Baseline Ingestor

Sources:
  namq_10_gdp    Quarterly National Accounts GDP + components
                   B1GQ chain-linked volumes (CLV10, million EUR, SCA)
                   B1GQ current prices (CP_MEUR, SCA)
                   P51G GFCF current prices (CP_MEUR, SCA)
  prc_hicp_minr  HICP annual rate of change, all items (monthly, coicop18=TOTAL)
                 prc_hicp_manr discontinued 2025-12 (ECOICOP reclassification)

Output: lekwankwa-historical-vault/product=global_macro/
            country={ISO3}/source=eurostat_sdmx/year=YYYY/month=MM/
            global_macro_data.parquet

Schema matches SCHEMA_GOLD_STANDARD imf_weo.json field names (same structure
as wages.json — macro observations with observed_value).
"""
...
def _build_macro_rows(df_raw: pd.DataFrame, cfg: dict) -> pd.DataFrame:
    if df_raw.empty:
        return pd.DataFrame()
    ...
    for _, r in df_raw.iterrows():
        ...
        rows.append({
            # Gold-standard imf_weo.json fields (same structure as wages.json)
            "data_vintage_id":       vid,
...
def _ingest_one(cfg: dict) -> int:
    ...
    metric_code = cfg["metric_code"]
    start       = cfg.get("start_period", "1995-Q1")
    ...
    df_vault = _build_macro_rows(df_raw, cfg)
...
def run() -> int:
    log.info("=" * 70)
    log.info("EUROSTAT — Global Macro Baseline")
    log.info(f"Series: {len(MACRO_CONFIGS)} | Countries: {len(ALL_GEO2)}")
    log.info("=" * 70)
    ...
    log.info(f"\nGlobal macro ingestion complete: {total:,} rows")
    return total
