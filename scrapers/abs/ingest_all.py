"""
ABS Australia full ingestion — all 5 vault products.

Fetches all series via ABS SDMX API, builds PIT-compliant vault rows,
and writes to Hive-partitioned parquet vault.

Usage:
    python -m scrapers.abs.ingest_all
"""
...
def _ingest_series(
    ...
    log.info("  ABS %s/%s (%s) -> %s", dataflow, key, metric_code, vault_product)

    df_raw = fetch_sdmx(
        dataflow, key, start_period=_START_PERIOD,
        series_key_filter=series_key_filter,
    )
    if df_raw.empty:
        log.warning("  SKIP %s/%s: no data", dataflow, key)
        return 0

    log.info("  %s/%s: %d obs", dataflow, key, len(df_raw))
...
def run() -> int:
    log.info("=" * 70)
    log.info("ABS AUS — All 5 vault products")
    log.info("Series: %d  |  pit_coverage_type: RELEASE_DATE_ONLY/structural_ceiling", len(SERIES))
    log.info("=" * 70)
    ...
    log.info("\nABS AUS ingestion complete: %d rows written", total)
    if total > 0:
