...
# Was backtesting/backtest_engine/config/SCHEMA_STANDARD.yaml — that whole
# folder is local-only test scaffolding, never committed/deployed, so this
# path always 404'd in production (silently falls back to an empty
# non-nullable set, degrading check C1 to a no-op). configs/ is the actual
# deployed config location already used for the GX validator configs.
_SCHEMA_PATH = _ROOT / "configs" / "SCHEMA_STANDARD.yaml"
