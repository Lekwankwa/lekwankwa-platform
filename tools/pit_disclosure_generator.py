...
def cloud_function_handler(event: dict, context: Any) -> None:  # type: ignore
    ...
    master = build_master()
    write_outputs(tmp_dir, master, gcs_bucket=metadata_bucket, gcs_prefix="pit_disclosure")
    logger.info("PIT disclosure updated in GCS: %s/pit_disclosure/", metadata_bucket)
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--gcs-prefix", default="pit_disclosure", metavar="PREFIX",
        help="GCS object prefix (default: pit_disclosure)",
    )
    p.add_argument(
