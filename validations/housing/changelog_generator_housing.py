"""
Changelog Generator for Hive Vault — Housing Supply & Shelter Inflation
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('changelog_generation_housing.log'),
...
def log_data_ingestion(df: pd.DataFrame, year: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "change_description": (
            f"Converted and loaded {len(df)} records from API to Parquet (Hive partition format)"
        ),
        ...
        "change_metadata": "source_format=api;target_format=parquet;compression=snappy"
    }
...
def log_quality_metrics(df: pd.DataFrame, year: int, outliers_count: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "change_author": "automated_pipeline",
        "change_metadata": (
            f"null_count={null_count};duplicates={duplicate_count};"
            f"outliers={outliers_count};completeness_pct={completeness_pct:.2f}"
        )
    }
...
def log_validation_result(df: pd.DataFrame, year: int) -> Dict[str, Any]:
    ...
    # Check 2: Required columns (housing schema)
    required_cols = ["series_id", "value", "data_timestamp", "conversion_timestamp"]
    ...
    if "value" in df.columns:
        if df["value"].isnull().sum() == 0:
            checks_passed.append("no_null_values")
        else:
            checks_failed.append("no_null_values")

    status = "passed" if not checks_failed else "failed"
    severity = "info" if status == "passed" else "critical"
    validation_desc = (
        f"Validation {status}: {len(checks_passed)} checks passed, {len(checks_failed)} failed"
    )
    ...
    return {
        ...
        "change_author": "automated_pipeline",
        "change_metadata": (
            f"checks_passed={','.join(checks_passed)};checks_failed={','.join(checks_failed)}"
        )
    }
...
def generate_changelog_for_year(year_folder: Path, year: int, source: str, previous_schema: Dict = None) -> Dict:
    ...
    try:
        ...
        changelog_entries = [
            log_data_ingestion(df, year),
            log_schema_tracking(df, year),
            log_quality_metrics(df, year, outliers_count),
            log_validation_result(df, year),
        ]

        if previous_schema:
            changelog_entries.extend(detect_schema_evolution(df, year, previous_schema))
...
def run_changelog_generation():
    logger.info("=" * 70)
    logger.info("HOUSING — CHANGELOG GENERATION PIPELINE")
    logger.info("=" * 70)
    ...
    for source in SOURCES:
        ...
        for year_folder in year_folders:
            ...
            result = generate_changelog_for_year(year_folder, year, source, previous_schema)
            if result["success"]:
                source_successful += 1
        ...
        logger.info(
            f"\n{source.upper()} Summary: {source_successful}/{len(year_folders)} years, "
            f"{source_entries} changelog entries"
        )
    ...
    for iso in EU27_ISO3:
        ...
        for year_dir in sorted(src.glob("year=*")):
            ...
            entries = [{
                ...
                "change_type": "data_ingestion",
                "change_category": "pipeline",
                "change_description": f"{iso}: {n_records:,} Eurostat SDMX housing records for year={year}",
                "change_severity": "info",
...
if __name__ == "__main__":
    ...
    sys.exit(0 if (_run_eu27_changelog() if _args.eu27 else run_changelog_generation()) else 1)
